...
> - manual — 사람이 직접 입력한 별칭 (현재 모든 데이터가 이 값)
> - auto — 시스템/스크립트가 자동 생성한 별칭 (예: 향후 크롤링이나 NLP 파이프라인 등으로 추가될 케이스)
> - 실용적 용도는 신뢰도 구분입니다. manual은 검수된 데이터, auto는 자동 생성이라 오류 가능성이 있으므로 시드 스크립트나 매칭 로직에서 가중치를 달리 줄
>   수 있도록 설계된 필드입니다.
