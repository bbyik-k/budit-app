...
> **[2026-08-25 재조사]** 문제 3의 원인 진단에 부정확한 부분이 확인됐다. 본문은 당시 기록이므로 그대로 두고, 문서 말미의 [「2026-08-25 재조사」](#2026-08-25-재조사) 섹션에 정정 내용을 정리했다.
...
본문은 문제 3을 "URL 길이 초과"로 서술했으나, `@supabase/postgrest-js` 소스를 확인한 결과 에러를 발생시킨 조건은 다르다.

`PostgrestBuilder.ts:408-419`
...
| 구분                    | 실제                                                                                                                                                |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| 트리거 조건             | `cause.name === 'HeadersOverflowError'` 또는 `cause.code === 'UND_ERR_HEADERS_OVERFLOW'` — **undici(Node.js fetch)가 던진 HTTP 헤더 오버플로 에러** |
| URL 길이 문장의 역할    | `urlLength > this.urlLengthLimit`일 때만 hint에 **조건부로 덧붙는 부속 정보**. 에러 발생 조건이 아니다                                              |
| `urlLengthLimit` 기본값 | **8,000** (`PostgrestBuilder.ts:141` — `builder.urlLengthLimit ?? 8000`)                                                                            |

GET 요청의 쿼리 스트링이 HTTP 요청 라인에 실리므로 긴 URL이 헤더 오버플로를 **유발**한 것은 맞다. 그러나 둘은 별개 개념이며, 본문의 제목·타임라인·회고가 전부 "URL 길이" 프레임으로 서술돼 1차 원인인 헤더 한계를 가리고 있다.

### 2. "10,101자"는 직접 측정한 값이 아니다

이 수치는 `postgrest-js`가 에러 hint에 **자동 삽입한 값**이다.

`PostgrestBuilder.ts:396-397`
...
`this.url`은 `URL` 객체(`:78`의 `protected url: URL`)이므로 `toString()`은 퍼센트 인코딩된 문자열을 반환한다. 따라서 **인코딩 후 길이가 맞다**. 다만 본문 어디에도 이 수치가 라이브러리 자동 생성값이라는 사실과 측정 절차가 기록돼 있지 않다.

### 3. 당시 값의 재현은 확인 불가

커밋 `03271e4`의 삭제부 코드(`toCsv` + `.or()`)를 그대로 재현해 현재 DB 값으로 계산했다.

| 시나리오                       | URL 길이     |
| ------------------------------ | ------------ |
| **성분 + 그룹 전체** (50 / 47) | **13,071자** |
| 성분만 (35 / 34)               | 9,781자      |
| 그룹만 (15 / 13)               | 3,469자      |

본문 서술대로 "성분명 + 그룹명"을 모두 포함하면 **13,071자**가 나오며 본문의 10,101과 **2,970 차이**가 난다. 10,101에 가장 가까운 것은 성분만 넣은 9,781자(차이 320)이나, 이는 본문의 "여기에 각 성분의 그룹명까지 추가하면"이라는 서술과 배치된다.

당시 그룹 매핑 상태와 성분 마스터 규모가 지금과 달랐을 수 있으나 판단 근거가 없어, **10,101이 어떤 입력으로 산출됐는지는 확인 불가**로 남긴다.

참고로 세 시나리오 모두 `urlLengthLimit` 8,000을 넘으므로, 어느 쪽이었든 hint의 URL 길이 문장은 나타났을 것이다.

### 4. 인코딩 배율 실측 — 7.115

본문 `한국어 성분명은 UTF-8 인코딩 시 글자당 3바이트`라는 서술은 사실이다. 한글 1자는 UTF-8 3바이트이고, 퍼센트 인코딩되면 `%XX%XX%XX` 9문자가 된다.

두 제품의 `termsA` / `termsB` 문자 구성:
...
| ---------------------------- | --------- |
| 인코딩 전 (`or` 표현식)      | 1,761자   |
| 인코딩 후 (`url.toString()`) | 13,071자  |
| **실측 배율**                | **7.115** |

인코딩 후 문자열은 퍼센트 이스케이프로 전부 ASCII가 되므로 **문자 수와 바이트 수가 같다.** 따라서 "10,101자"와 "10,101바이트"는 같은 값을 가리킨다.

### 5. "typically 16KB"는 hint 문구이며 서버 설정 확인 기록이 없다

본문 `Supabase REST API (PostgREST)의 HTTP 헤더 한계(~16KB)를 초과`는 방향이 맞으나, 이 `16KB`는 라이브러리 hint의 `typically 16KB`를 그대로 옮긴 값이다. **실제 Supabase 서버의 헤더 한계 설정값을 확인한 기록은 본문에 없고, 재조사 시점에도 확인하지 못했다.**

### 6. toCsv의 괄호·쉼표 치환은 인코딩 관점에서 불필요했다

삭제부의 주석은 이 치환을 "괄호·쉼표 제거로 파싱 오류 방지"로 설명한다.
...
그러나 `postgrest-js`가 `URL.searchParams.set()`을 쓰므로 `toString()` 시점에 `(` → `%28`, `)` → `%29`, `,` → `%2C`로 자동 인코딩된다. **파싱 관점에서는 이 치환이 필요하지 않았다.**

그럼에도 이 치환이 `1,2-헥산다이올`을 `1 2-헥산다이올`로 변조해, 해당 성분이 포함된 충돌 규칙이 절대 매칭되지 않는 부가 버그를 낳았다(본문 「문제 3」의 ② 항목).

### 재조사 방법 및 한계

`termsA` / `termsB`는 현재 DB의 `product_ingredients` ⋈ `ingredients` ⋈ `ingredient_group_members`로 재구성했고, 계산은 Node.js `URL` 객체로 삭제부 코드를 그대로 재현해 수행했다.

설치된 `postgrest-js`는 **2.103.2**로, 사건 발생 시점(2026-04-21)의 버전과 다를 수 있다. `package.json`이 `"@supabase/supabase-js": "latest"`로 선언돼 있어 당시 버전은 **확인 불가**다. 다만 본문에 인용된 hint 문구가 현재 소스의 문자열과 일치하므로 해당 코드 경로는 당시에도 동일했다고 판단했다.

**RPC 전환이라는 해결책 자체는 타당하다.** POST 본문으로 배열을 넘기므로 URL 길이와 헤더 크기 양쪽 모두와 무관해진다.
