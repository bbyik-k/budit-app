...
- `ingredients.csv`: KCIA 기준 21,805개 성분 (당초 계획 / 현재 실적 162종은 아래 7장 참조)
- `products.csv`: 올리브영 상위 120개 제품 (당초 계획 / 현재 실적 362건은 아래 7장 참조)
...
### 제품 DB: 올리브영 상위 362건 (당초 계획 120건)

작성 당시 계획은 올리브영 랭킹 기준 4개 카테고리에서 각 30개씩 총 120개였습니다.
실제로는 소분류 단위로 수집·적재가 진행되어 **2026-08-25 기준 362건**이며, 카테고리별 수량이 균등하지 않습니다 (skincare 122 · cleansing 118 · suncare 96 · mask_pack 26).

| 카테고리  | 당초 계획 | 실제 적재 (2026-08-25) |
| --------- | --------- | ---------------------- |
| skincare  | 상위 30개 | 122건                  |
| mask_pack | 상위 30개 | 26건                   |
| cleansing | 상위 30개 | 118건                  |
| suncare   | 상위 30개 | 96건                   |
| **합계**  | **120개** | **362건**              |

각 제품의 전성분 텍스트를 올리브영 상품 상세 페이지 > 상품정보 제공고시에서 직접 수집했습니다.

### 성분 DB: KCIA 기준, 현재 162종

대한화장품협회(KCIA) 화장품성분사전 전체 규모는 약 21,805개이며, 이를 목표로 삼았습니다.
**2026-08-25 기준 실제 적재는 162종**으로 전체의 부분집합입니다. 한국어 표준명(`name`)과 영문 INCI명(`name_en`)을 함께 저장합니다.

### 충돌 규칙 DB: The Ordinary 충돌 차트 기반 52개 규칙

The Ordinary의 공식 성분 충돌 차트를 기반으로 핵심 규칙을 구축했습니다. 2026-08-25 기준 **26개 성분 그룹**과 **52개 충돌/시너지 규칙**(충돌 23 + 시너지 29)이 등록되어 있습니다.
